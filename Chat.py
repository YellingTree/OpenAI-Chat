...
# Define ANSI escape sequences for light blue color
if platform.system() == "Windows":
    # Windows requires special handling for ANSI escape sequences
    os.system("")
    LIGHT_BLUE = ""
    RESET = ""
else:
...
while next:
    ...
    if prompt == "end":
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo", messages=messages, max_tokens=3096)
        data = response['choices'][0]['message']['content']
        print(f"{LIGHT_BLUE}AI: {data}{RESET}")
        print()
